...
from ..integrations.qbittorrent import qb_file_id, qb_file_priority, qb_login
from .torrent_naming import (
...
def find_completed_download_name(
    ...
    try:
        ...
        for item in payload:
            ...
            if name and query_matches_torrent_name(
                query=query, torrent_name=name, season=season
            ):
                return name
    ...
    try:
        ...
        while time.monotonic() <= deadline:
            ...
            for row in files:
                ...
                if fid is None:
                    continue
                file_season = extract_season_number(str(row.get("name") or ""))
    ...
    try:
        ...
        for t in torrents:
            ...
            if not torrent_name or not torrent_hash:
                continue
            if not is_multi_season_pack(torrent_name):
                continue
            if not query_matches_torrent_name(
                query=query, torrent_name=torrent_name, season=None
            ):
                continue
            files_resp = http.get(
                urljoin(f"{s.qbittorrent_base}/", "api/v2/torrents/files"),
            ...
            season_files = [
                ...
                and season_path_matches(str(row.get("name") or ""), season)
            ]
            if not season_files:
                continue
